...
function loadFallbackData() {
    console.warn('Using fallback sample data since API is unavailable');
    ...
    const paginationInfo = {
        page: 1,
        limit: 10,
        total: sampleDrivers.length,
        total_pages: 1
    };
    
    const statsInfo = {
        total_drivers: sampleDrivers.length,
        active_drivers: sampleDrivers.filter(driver => driver.status === 'active').length,
        expiring_licenses: 2,
        incident_involved: sampleDrivers.filter(driver => driver.violations > 0).length
    };
    
    showNotification('Using demo data. To fix API issues, run setup_drivers.php in your browser.', 'warning');
...
function checkApiStatus() {
    // Add a flag to track API check completion
    ...
    const apiTimeout = setTimeout(() => {
        if (!apiCheckComplete) {
            ...
            if (headerEl) {
                ...
                warningContainer.className = 'bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-6';
                warningContainer.setAttribute('role', 'alert');
                warningContainer.innerHTML = `
                    <div class="flex items-center">
                        <i class="fas fa-exclamation-triangle mr-2"></i>
                        <div>
                            <p class="font-bold">API Timeout</p>
                            <p class="text-sm">The server is taking too long to respond. Using fallback data.</p>
                        </div>
                    </div>
                `;
                headerEl.parentNode.insertBefore(warningContainer, headerEl.nextSibling);
            }
        }
    }, 5000); // 5 second timeout
    
    fetch('api/driver_status_check.php')
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
            }
            ...
            if (data.status === 'error') {
                ...
                if (headerEl) {
                    ...
                    errorContainer.innerHTML = `
                        ...
                        <div class="mt-3">
                            <a href="setup_drivers.php" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                                Run Setup
                            </a>
                            <button id="showDetailsBtn" class="ml-2 text-red-500 underline text-sm">Show details</button>
                        </div>
                        <div id="errorDetails" class="mt-3 hidden">
                            <pre class="text-xs bg-gray-100 p-2 overflow-x-auto">${JSON.stringify(data.checks, null, 2)}</pre>
                        </div>
                    `;
                    
                    // Insert after header
                    headerEl.parentNode.insertBefore(errorContainer, headerEl.nextSibling);
                ...
                // Use fallback data instead of calling loadDrivers
                loadFallbackData();
                ...
                if (headerEl) {
                    ...
                    warningContainer.innerHTML = `
                        ...
                        <button id="showWarningDetailsBtn" class="mt-1 text-yellow-700 underline text-sm">Show details</button>
                        <div id="warningDetails" class="mt-3 hidden">
                            <pre class="text-xs bg-gray-100 p-2 overflow-x-auto">${JSON.stringify(data.checks, null, 2)}</pre>
                        </div>
                    `;
                    ...
                    // Add event listener for details toggle
                    const showWarningDetailsBtn = document.getElementById('showWarningDetailsBtn');
                    if (showWarningDetailsBtn) {
                        showWarningDetailsBtn.addEventListener('click', () => {
                            const details = document.getElementById('warningDetails');
                            if (details) {
                                details.classList.toggle('hidden');
                                showWarningDetailsBtn.textContent = 
                                    details.classList.contains('hidden') ? 'Show details' : 'Hide details';
                            }
                        });
                    }
                ...
                // Continue loading actual data
                loadDrivers();
            } else {
                // Everything is good, load drivers normally
                loadDrivers();
            }
        })
        .catch(error => {
            ...
            // Add a specific error message for the user
            const headerEl = document.querySelector('h1');
            if (headerEl) {
                const errorContainer = document.createElement('div');
                errorContainer.className = 'bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-6';
                errorContainer.innerHTML = `
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-sm">Cannot connect to the server. Using fallback data.</p>
